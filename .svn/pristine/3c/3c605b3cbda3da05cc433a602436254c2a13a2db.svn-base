	...
	export default {
		...
		methods: {
			...
			setAxios(data) {				/* 记录用户所有信息*/
				this.user = data;
				/**存储用户信息**/
				if(!sessionStorage.getItem('user') && !sessionStorage.getItem('user3')) sessionStorage.setItem('user', JSON.stringify(data));
				if (window.location.hash.split('/')[1] != 'formProcessCorp') {
					// axios.defaults.headers.common['token'] = this.user == null ? '' : this.user.token;
					// axios.defaults.headers.common['token'] = '000000';//3.0版写死
				}else{//外部报单页面
		...
		created() {
			if(this.getQueryString('os') == "app") {
				// 3.0登陆传参 token写死、userid和corpid移动端传入
				if (this.getQueryString('userid') && this.getQueryString('corpid') && this.getQueryString('token') === '000000' ) {
					let userEntity = {
						userid:this.getQueryString('userid'),
						corpid:this.getQueryString('corpid')
					}
					sessionStorage.setItem('user3', JSON.stringify(userEntity) );
					this.setAxios(JSON.parse(sessionStorage.getItem('user3')));
				};
